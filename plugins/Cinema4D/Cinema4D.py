    ...
    def RenderExecutable( self ):
        ...
        if SystemUtils.IsRunningOnWindows():
            if build == "32bit":
                ...
                if self.C4DExe == "":
                    self.LogWarning( "32 bit Cinema 4D " + str(version) + " render executable was not found in the semicolon separated list \"" + C4DExeList + "\".  Checking for any executable that exists instead." )
            elif build == "64bit":
                self.LogInfo( "Enforcing 64 bit build of Cinema 4D" )
                self.C4DExe = FileUtils.SearchFileListFor64Bit( C4DExeList )
                if self.C4DExe == "":
                    self.LogWarning( "64 bit Cinema 4D " + str(version) + " render executable was not found in the semicolon separated list \"" + C4DExeList + "\".  Checking for any executable that exists instead." )

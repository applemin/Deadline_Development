...
class MaxPlugin( DeadlinePlugin ):
        ...
        if self.IsDBRJob:
            ...
            if self.GetCurrentTaskId() == "0":
                ...
                if self.VrayDBRJob:
                    ...
                    self.LogInfo( self.Prefix + "Transfer Missing Assets: %s" % self.MyMaxController.VrayDBRTransferMissingAssets )
                    ...
                    self.LogInfo( self.Prefix + "Auto-Start Local Slave: %s" % self.MyMaxController.VrayRtDBRAutoStartLocalSlave )
                elif self.MentalRayDBRJob:
                    self.MyMaxController.GetMentalRaySettings()
                    self.LogInfo( self.Prefix + "Satellite Port Number: %s" % self.MyMaxController.MentalRaySatPortNumber )
                elif self.CoronaDBRJob:
                ...
                if (self.VrayDBRJob and self.MyMaxController.VrayDBRDynamicStart and self.MyMaxController.VrayDBRUseLocalMachine) or \
                ...
                elif (self.VrayDBRJob and self.MyMaxController.VrayDBRDynamicStart and not self.MyMaxController.VrayDBRUseLocalMachine) or \
                ...
                self.LogInfo( self.Prefix + "Use IP Address instead of Hostname: %s" % self.MyMaxController.DBRUseIPAddresses )
                ...
                backupConfigFile = os.path.join( self.GetJobsDataDirectory(), os.path.basename( self.MyMaxController.DBRConfigFile ) )
                ...
                if self.VrayDBRJob:
                    self.MyMaxController.UpdateVrayDBRConfigFile( self.MyMaxController.DBRMachines, self.MyMaxController.DBRConfigFile )
                elif self.VrayRtDBRJob:
                    self.MyMaxController.UpdateVrayRtDBRConfigFile( self.MyMaxController.DBRMachines, self.MyMaxController.DBRConfigFile )
                elif self.MentalRayDBRJob:
                    self.MyMaxController.UpdateMentalRayConfigFile( self.MyMaxController.DBRMachines, self.MyMaxController.DBRConfigFile )
                elif self.CoronaDBRJob:
                    self.MyMaxController.UpdateCoronaConfigFile( self.MyMaxController.DBRMachines, self.MyMaxController.DBRConfigFile )
                ...
                if( os.path.isfile( backupConfigFile ) ):
                    self.LogInfo( self.Prefix + "Restoring backup config file: %s to original location: %s" % ( backupConfigFile, self.MyMaxController.DBRConfigFile )  )
                ...
                if self.VrayDBRJob or self.VrayRtDBRJob:
                    ...
                    self.LogInfo( self.Prefix + "Launching Corona DR process and waiting for Corona DR render to complete" )
    ...
    def MonitoredManagedProcessExit( self, name ):
        ...
        if maxNetworkLog.find( "in Init. nrGetIface() failed" ) >= 0:
            self.FailRender( "Monitored managed process \"" + name + "\" has exited because an error occurred while initializing Backburner. Please install or upgrade Backburner on this machine and try again.\n" + maxNetworkLog )
        else:
...
class MaxController( object ):
    ...
    def Initialize( self ):
        ...
        if SystemUtils.IsRunningOnWindows():
            if self.ForceBuild == "32bit":
                ...
                if self.MaxRenderExecutable == "":
                    self.Plugin.LogWarning( "No 32 bit 3ds Max" + maxEdition + " " + str(self.Version) + " render executable found in the semicolon separated list \"" + renderExecutableList + "\". Checking for any executable that exists instead." )
            elif self.ForceBuild == "64bit":
                self.MaxRenderExecutable = FileUtils.SearchFileListFor64Bit( renderExecutableList )
                if self.MaxRenderExecutable == "":
                    self.Plugin.LogWarning( "No 64 bit 3ds Max" + maxEdition + " " + str(self.Version) + " render executable found in the semicolon separated list \"" + renderExecutableList + "\". Checking for any executable that exists instead." )
            
        if self.MaxRenderExecutable == "":
            self.MaxRenderExecutable = FileUtils.SearchFileList( renderExecutableList )
            if self.MaxRenderExecutable == "":
                self.Plugin.FailRender( "No 3ds Max" + maxEdition + " " + str(self.Version) + " render executable found in the semicolon separated list \"" + renderExecutableList + "\". The path to the render executable can be configured from the Plugin Configuration in the Deadline Monitor." )
        ...
        if self.Version < 2013:
            ...
            if languageOverride == "Default":
                ...
                if languageCode == "ENU":
                    ...
                    self.Plugin.LogInfo( "Language code override \"" + languageOverride + "\" is not supported. Defaulting to ENU (English)" )
                    languageCode = "ENU"
                    languageSubDir = "en-US"
        ...
        if( self.Plugin.GetBooleanPluginInfoEntryWithDefault( "OverrideFailOnExistingMaxProcess", False ) ):
            self.FailOnExistingMaxProcess = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "FailOnExistingMaxProcess", True )
            self.Plugin.LogInfo( "Fail on existing 3dsmax process: %d (from plugin info override)" % self.FailOnExistingMaxProcess )
        else:
            self.FailOnExistingMaxProcess = self.Plugin.GetBooleanConfigEntryWithDefault( "FailOnExistingMaxProcess", True )
        ...
        self.DisableProgressUpdateTimeout = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "DisableProgressUpdateTimeout", False )
        ...
        if( not os.path.isfile( Path.ChangeExtension( self.MaxStartupFile, ".xml" ) ) ):
            self.Plugin.FailRender( "The Backburner xml job file %s does not exist" % Path.ChangeExtension( self.MaxStartupFile, ".xml" ) )
        ...
        if self.UseUserProfiles:
            if not self.IsMaxDesign:
                ...
                if( self.Is64Bit ):
                    ...
                    self.UserProfileDataPath = os.path.join( PathUtils.GetLocalApplicationDataPath(), "Autodesk\\" + maxDirName + "\\" + str(self.Version) + " - 64bit\\" + self.LanguageCodeStr )
                else:
                    self.UserProfileDataPath = os.path.join( PathUtils.GetLocalApplicationDataPath(), "Autodesk\\3dsmax\\" + str(self.Version) + " - 32bit\\" + self.LanguageCodeStr )
            else:
                # The user profile directory is different for the Design version
                if( self.Is64Bit ):
                    self.UserProfileDataPath = os.path.join( PathUtils.GetLocalApplicationDataPath(), "Autodesk\\3dsMaxDesign\\" + str(self.Version) + " - 64bit\\" + self.LanguageCodeStr )
                else:
                    self.UserProfileDataPath = os.path.join( PathUtils.GetLocalApplicationDataPath(), "Autodesk\\3dsMaxDesign\\" + str(self.Version) + " - 32bit\\" + self.LanguageCodeStr )
        ...
        if self.KillWsCommCntrProcesses:
            try:
                ...
                self.Plugin.LogWarning( "Could not copy %s to %s" % (self.pluginStartupKillScript, self.StartupKillScript) )
                self.Plugin.LogInfo( "If this fails, make sure that the necessary permissions are set on this destination folder to allow for this copy to take place" )
        ...
        if self.Version >= 2015:
            if self.UseUserProfiles:
                ...
                self.WorkspacePath = PathUtils.ChangeFilename( self.MaxRenderExecutable, self.LanguageSubDir+"\\UI\\Workspaces" )
        ...
        if( self.MaxPluginIni != "" ):
            ...
            if( self.MaxPluginIni != "" ):
                ...
                if self.Version < 2013:
                    self.MaxPluginIni = os.path.join( Path.GetDirectoryName( self.MaxRenderExecutable ), "pluginnetrender.ini" )
                    if( not os.path.isfile( self.MaxPluginIni ) ):
                        self.MaxPluginIni = os.path.join( Path.GetDirectoryName( self.MaxRenderExecutable ), "plugin.ini" )
                        if( not os.path.isfile( self.MaxPluginIni ) ):
                            self.Plugin.FailRender( "The plugin ini file %s does not exist, and no alternative plugin.ini file was provided" % self.MaxPluginIni )
                else:
                    self.MaxPluginIni = os.path.join( os.path.join( Path.GetDirectoryName( self.MaxRenderExecutable ), self.LanguageSubDir ), "pluginnetrender.ini" )
                    if( not os.path.isfile( self.MaxPluginIni ) ):
                        self.MaxPluginIni =  os.path.join( os.path.join( Path.GetDirectoryName( self.MaxRenderExecutable ), self.LanguageSubDir ), "plugin.ini" )
                        if( not os.path.isfile( self.MaxPluginIni ) ):
                            self.Plugin.FailRender( "The plugin ini file %s does not exist, and no alternative plugin.ini file was provided" % self.MaxPluginIni )
        ...
        if self.UsingCustomPluginIni:
            ...
            if( self.UseUserProfiles ):
                ...
                if( os.path.isfile( self.UserPluginIni ) ):
                    ...
                    self.Plugin.LogInfo( "Not including user profile plugin ini because it does not exist: %s" % self.UserPluginIni )
            else:
                self.UserPluginIni = os.path.join( self.MaxDataPath, "Plugin.UserSettings.ini" )
                if( os.path.isfile( self.UserPluginIni ) ):
                    self.Plugin.LogInfo( "Including user profile plugin ini: %s" % self.UserPluginIni )
        ...
        if( self.Is64Bit ):
            self.LightningPluginFile = os.path.join( self.Plugin.MaxStartupDir, "lightning64Max" + str(self.Version) + ".dlx" )
        else:
            self.LightningPluginFile = os.path.join( self.Plugin.MaxStartupDir, "lightningMax" + str(self.Version) + ".dlx" )
    ...
    def StartMax( self ):
        if( self.FailOnExistingMaxProcess ):
            ...
            if( ProcessUtils.IsProcessRunning( processName ) ):
                ...
                self.Plugin.FailRender( "FailOnExistingMaxProcess is enabled, and a process %s with pid %d exists - shut down this copy of 3dsmax to enable network rendering on this machine" % (processName, process.Id) )
        ...
        if( os.path.isfile( self.MaxIni ) ):
            if( FileUtils.GetIniFileSetting( self.MaxIni, "MAXScript", "LoadSaveSceneScripts", "1" ) != "1" ):
                self.Plugin.FailRender( "3dsmax.ini setting in [MAXScript], LoadSaveSceneScripts, is disabled - it must be enabled for deadline to render with 3dsmax" )
        else:
        ...
        if self.Version < 2013:
            ...
            self.Plugin.LogInfo( "If this fails, make sure that the necessary permissions are set on this folder to allow for this copy to take place" )
        ...
        try:
            ...
            self.Plugin.FailRender( "Timed out waiting for the lightning version to be sent - check the install of lightningMax.dlx\n%s" % self.NetworkLogGet() )
    ...
    def StartMaxJob( self ):
        ...
        if( self.MaxScriptJob ):
            ...
            if( not os.path.isfile( self.MaxScriptJobScript ) ):
                self.Plugin.FailRender( "The maxscript submitted with the MAXScript job, \"%s\", does not exist" % self.MaxScriptJobScript )
        ...
        if self.RegionRenderingSingleJob:
            self.RenderOutputOverride = self.Plugin.GetPluginInfoEntryWithDefault( "RegionFilename" + self.RegionRenderingIndex, "" ).strip()
        else:
        ...
        if self.Plugin.GetBooleanPluginInfoEntryWithDefault( "SaveFile", True ) == self.Plugin.GetBooleanPluginInfoEntryWithDefault( "SaveFile", False ):
        ...
        self.IgnoreMissingExternalFiles = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "IgnoreMissingExternalFiles", True )
        ...
        elementIndex = 0
        while True:
            elementToIgnore = self.Plugin.GetPluginInfoEntryWithDefault( "IgnoreRenderElementsByName" + str(elementIndex), "" )
            if elementToIgnore != "":
        ...
        self.BlackFramesCheckRenderElements = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "BlackFramesCheckRenderElements", False )
        ...
        if( self.FailOnBlackFrames ):
            ...
            self.Plugin.LogInfo( "Fail on black frames enabled: black pixel percentage = %d, black pixel threshold = %.4f, check render elements = %s" % (self.BlackPixelPercentage, self.BlackPixelThreshold,self.BlackFramesCheckRenderElements) )
        ...
        if self.RegionRenderingSingleJob:
            if not self.RegionAnimation:
                self.RegionLeft = self.Plugin.GetIntegerPluginInfoEntryWithDefault( "RegionLeft" + self.RegionRenderingIndex, 0 )
                self.RegionTop = self.Plugin.GetIntegerPluginInfoEntryWithDefault( "RegionTop" + self.RegionRenderingIndex, 0 )
                self.RegionRight = self.Plugin.GetIntegerPluginInfoEntryWithDefault( "RegionRight" + self.RegionRenderingIndex, 0 )
                self.RegionBottom = self.Plugin.GetIntegerPluginInfoEntryWithDefault( "RegionBottom" + self.RegionRenderingIndex, 0 )
            else:
                self.RegionLeftArray = self.Plugin.GetPluginInfoEntryWithDefault( "RegionLeft"+ self.RegionRenderingIndex,"0" )
                self.RegionTopArray = self.Plugin.GetPluginInfoEntryWithDefault( "RegionTop"+ self.RegionRenderingIndex,"0" )
                self.RegionRightArray = self.Plugin.GetPluginInfoEntryWithDefault( "RegionRight"+ self.RegionRenderingIndex,"0" )
                self.RegionBottomArray = self.Plugin.GetPluginInfoEntryWithDefault( "RegionBottom"+ self.RegionRenderingIndex,"0" )
        ...
        if( self.RegionRendering ):
            self.Plugin.LogInfo( "Region rendering enabled: left = %d, top = %d, right = %d, bottom = %d, padding = %d, type = %s" % (self.RegionLeft, self.RegionTop, self.RegionRight, self.RegionBottom, self.RegionPadding, self.RegionType) )
        ...
        if os.path.isfile( pathMappingScript ) and pathMapAssets:
            self.MaxSocket.Send( "SlaveFolders,\"" + self.Plugin.GetJobsDataDirectory().replace( "\\", "/" ) + "\",\"" + self.Plugin.GetPluginDirectory().replace( "\\", "/" ) + "\"" )
    ...
    def CreateStartupScript( self, port ):
        ...
        if len(selectedGPUs) > 0:
            ...
            self.Plugin.LogInfo( "This Slave is overriding its GPU affinity, so the following GPUs will be used by Octane/RedShift/V-Ray RT: %s" % gpus )
            self.Plugin.SetProcessEnvironmentVariable( "DEADLINE_GPU_AFFINITY", "#(" + gpus + ")" ) # Octane / Redshift (prior to 2.0.91-prod or prior to 2.5.11-beta)
    ...
    def CopyLightningDlx( self ):
        ...
        if( os.path.isfile( newLightningPluginFile ) ):
            try:
                ...
                self.Plugin.LogWarning( "Could not delete old %s - this file may be locked by another copy of 3dsmax" % newLightningPluginFile )
        else:
            shutil.copy2( self.LightningPluginFile, newLightningPluginFile )
    ...
    def CreatePluginInis( self, lightningDir ):
        ...
        self.TempLightningIni = os.path.join( PathUtils.GetSystemTempPath(), "lightningplugin_" + self.AuthentificationToken + ".ini" )
        ...
        self.TempPluginIni = os.path.join( PathUtils.ToShortPathName( PathUtils.GetSystemTempPath() ), "dl_" + self.AuthentificationToken + ".ini" )
    ...
    def WaitForConnection( self, errorMessageOperation ):
        ...
        while( DateTime.Now.Subtract( startTime ).TotalSeconds < self.LoadMaxTimeout and not self.MaxSocket.IsConnected and not self.Plugin.IsCanceled() ):
            try:
                self.Plugin.VerifyMonitoredManagedProcess( self.ProgramName )
        ...
        if( not self.MaxSocket.IsConnected ):
            if( DateTime.Now.Subtract( startTime ).TotalSeconds < self.LoadMaxTimeout ):
                self.Plugin.FailRender( "%s: Max exited unexpectedly - check that max starts up with no dialog messages\n%s" % (errorMessageOperation, self.NetworkLogGet()) )
            else:
                self.Plugin.FailRender( "%s: Timed out waiting for 3ds max to start - consider increasing the LoadMaxTimeout in the 3dsmax plugin configuration (current value is %d seconds).\n%s" % (errorMessageOperation, self.LoadMaxTimeout, self.NetworkLogGet()) )
        
        if( receivedToken != self.AuthentificationToken ):
            self.Plugin.FailRender( "%s: Did not receive expected token from Lightning plugin (got \"%s\") - an unexpected error may have occurred during initialization\n%s" % (errorMessageOperation, receivedToken, self.NetworkLogGet()) )
    ...
    def LoadMaxFile( self ):
        ...
        while( DateTime.Now.Subtract( startTime ).TotalSeconds < self.StartJobTimeout and response == "" and not self.Plugin.IsCanceled() ):
            self.Plugin.VerifyMonitoredManagedProcess( self.ProgramName )
            self.Plugin.FlushMonitoredManagedProcessStdout( self.ProgramName )
        ...
        if( response == "" ):
            self.Plugin.FailRender( "Timed out waiting for the 3dsmax scene file to load - likely caused by ADSK Comms Center, 3rd party plugin/script " \
    ...
    def RenderFrame( self, frameNumber ):
        ...
        if self.RegionRendering and self.RegionRenderingSingleJob:
            ...
            self.RenderOutputOverride = self.Plugin.GetPluginInfoEntryWithDefault( "RegionFilename" + str(self.RegionRenderingIndex), "" ).strip()
            if self.RenderOutputOverride != "":
                self.RenderOutputOverride = RepositoryUtils.CheckPathMapping( self.RenderOutputOverride ).replace( "/", "\\" )
            ...
            reRegionFilename = self.Plugin.GetPluginInfoEntryWithDefault( "RegionReFilename" + self.RegionRenderingIndex + "_" + str(reRegionFilenameIndex), "" )
            ...
            if VrayRawBufferFile:
                filePath = self.Plugin.GetPluginInfoEntryWithDefault( "RawBufferFilename"+ str(self.RegionRenderingIndex), "" )
                self.SetVrayRawBufferFile( filePath )

            if VraySplitBufferFile:
                filePath = self.Plugin.GetPluginInfoEntryWithDefault( "SplitBufferFilename"+ str(self.RegionRenderingIndex), "" )
        ...
        if( self.MaxScriptJob ):
            ...
            try:
                ...
                self.Plugin.FailRender( "RenderFrame: Timed out waiting for the lightning 3dsmax plugin to acknowledge the RenderTask command.\n%s" % self.NetworkLogGet() )
            ...
            if( not response.startswith( "STARTED" ) ):
                self.Plugin.FailRender( "RenderFrame: Did not receive a started message in response to RenderTask - got \"%s\"\n%s" % (response, self.NetworkLogGet()) )
                ...
                if self.RegionRenderingSingleJob:
                    ...
                    for component in range( len(outputTileFileNames) ):
                        for tile in range( len(outputTileFileNames[component]) ):
                            ...
                            self.Plugin.LogInfo( "Searching local output folder for: " + os.path.join( self.TempFolder, replacedFileName) )
                            if os.path.isfile( os.path.join( self.TempFolder, replacedFileName) ):
                                self.Plugin.LogInfo( "Found " + os.path.join( self.TempFolder, replacedFileName)+ " copying to "+ os.path.join( destinationFolder, replacedFileName) )
                                self.Plugin.VerifyAndMoveFile( os.path.join( self.TempFolder, replacedFileName), os.path.join( destinationFolder, replacedFileName), -1 )
                            else:
                                self.Plugin.LogWarning( "Unable to locate local render file: " + os.path.join( self.TempFolder, replacedFileName) )
                else:
            ...
            if (VraySplitBufferFile and self.RegionPadding > 0):
                ...
                while True:
                    ...
                    with open( tempFile, "w" ) as text_file:
                        ...
                        fileExtension = ""
                        if not self.RegionAnimation:
                            splitBufferFile = self.Plugin.GetPluginInfoEntryWithDefault( "SplitBufferFilename"+str(self.Plugin.GetCurrentTaskId()), "" )
                        else:
                        ...
                        includeReName = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "RenderElementsIncludeNameInPath", False )
                        includeReType = self.Plugin.GetBooleanPluginInfoEntryWithDefault( "RenderElementsIncludeTypeInPath", False )
                    ...
                    proc = subprocess.Popen([pythonPath, tempFile, self.DraftDirectory], startupinfo=startupinfo, stdout=subprocess.PIPE, stderr=subprocess.PIPE)              
            ...
            if (VrayRawBufferFile and self.RegionPadding > 0):
                ...
                with open( tempFile, "w" ) as text_file:
                    ...
                    fileExtension = ""
                    if not self.RegionAnimation:
                        fileName, fileExtension = os.path.splitext(self.Plugin.GetPluginInfoEntryWithDefault( "RawBufferFilename"+str(self.Plugin.GetCurrentTaskId()), "" ).replace("\\","\\\\"))
                    else:
                        fileName, fileExtension = os.path.splitext(self.Plugin.GetPluginInfoEntryWithDefault( "RawBufferFilename0", "" ).replace("\\","\\\\"))
    ...
    def ExecuteMaxScriptFile( self, maxscriptFilename, **keywordArgs ):
        ...
        requestMessage = "ExecuteMaxScriptFile," + maxscriptFilename.replace( "\\", "/" ) + "," + str(useFrameNumber) + "," + str(frameNumberOverride) + "," + str(returnAsString)
                ...
                if( isinstance( e, SimpleSocketTimeoutException ) ):
                    if timeoutEnabled and DateTime.Now.Subtract( startTime ).TotalSeconds >= progressTimeout:
                        if timeoutOverride < 0:
                            ...
                            self.Plugin.FailRender( "Timed out during script execution. Current value is %d seconds.\n%s" % (progressTimeout, self.NetworkLogGet()) )
    ...
    def LightingFunctionHandler( self, request ):
        ...
        if( match.Success ):
            try:
                return "SUCCESS: " + self.Plugin.GetJobInfoEntryElement( match.Groups[ 2 ].Value, Convert.ToInt32( match.Groups[ 1 ].Value ) )
            except:
                return "NOVALUE"
        ...
        if( match.Success ):
            ...
            # Unknown message.
            self.Plugin.FailRender( "Got unexpected request from the lightning max plugin: \"" + request + "\"\n" + self.NetworkLogGet() )
            return "CANCEL"
...
class MaxProcess( ManagedProcess ):
    ...
    def InitializeProcess( self ):
        ...
        self.AddPopupHandler( ".*Customer Involvement Program.*", "Participate &anonymously ;Participate &Anonymously;No, thanks.;OK" )
    ...
    def SetupCPUAffinity( self ):
        ...
        if self.MaxController.Plugin.OverrideCpuAffinity():
            overrideMessage.insert(0, "The Slave is overriding its CPU affinity.")
        ...
        if threads:
            ...
            #If the slave has multiple render threads we want to reduce as much overlap as we can.
...
class CoronaDRProcess( ManagedProcess ):
    ...
    def RenderExecutable( self ):
        ...
        coronaDrExeList = self.Plugin.GetConfigEntry( "CoronaDrServerExecutable" )
        executable = FileUtils.SearchFileList( coronaDrExeList )
        if( executable == "" ):
            self.Plugin.FailRender( "Corona DrServer executable was not found in the semicolon separated list \"" + coronaDrExeList + "\". The path to the render executable can be configured from the Plugin Configuration in the Deadline Monitor." )
        else:
        ...
        if( ProcessUtils.IsProcessRunning( processName ) ):
            ...
            if len( processes ) > 0:
                ...
                if( existingDRProcess == "Fail On Existing Process" ):
                    if process != None:
                        self.Plugin.FailRender( "Fail On Existing Process is enabled, and a process '%s' with pid '%d' exists - shut down this copy of Corona DrServer. Ensure DrServer is NOT already running!" % (processName, process.Id) )

                if( existingDRProcess == "Kill On Existing Process" ):
                    if( ProcessUtils.KillProcesses( processName, 3 ) ):
                        if process != None:
                            self.Plugin.LogInfo( "Successfully Killed Corona DrServer process: '%s' with pid: '%d'" % (processName, process.Id) )

                        SystemUtils.Sleep(5000)

                        if( ProcessUtils.IsProcessRunning( processName ) ):
                            self.Plugin.LogWarning( "Corona DrServer is still running: '%s' process, perhaps due to it being automatically restarted after the previous kill command. Ensure Corona DrServer is NOT already running!" % processName )
                            process = Process.GetProcessesByName( processName )[ 0 ]
                            if process != None:
                                self.Plugin.FailRender( "Kill On Existing Process is enabled, and a process '%s' with pid '%d' still exists after executing a kill command. Ensure Corona DrServer is NOT already running!" % (processName, process.Id) )

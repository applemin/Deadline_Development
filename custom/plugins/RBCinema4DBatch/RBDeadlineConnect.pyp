...
def runPathMapping( deadlineTemp, texPathFilename ):
    ...
    if texPathFilename:
        print( "Setting Texture Paths" )
        with io.open( texPathFilename, mode="r", encoding="utf-8" ) as texPathFile:
            for index, line in enumerate( texPathFile ):
                # The display of texture search paths is 1-indexed, but actually setting the value is 0-indexed.
                print( 'Setting texture path %s to "%s"' % ( index+1, line.strip() ) )
                c4d.SetGlobalTexturePath( index, line.strip() )
    ...
    with open(pathMapFilename, "w") as pathMapFile:
        for obj, paramid in objectsWithPaths:
            pathMapFile.write(obj[paramid].strip() + "\n")
    ...
    for bc, paramid, groupid in description:                # Iterate over the parameters of the description
        if paramid.GetDepth() > 0 and paramid[0].dtype == c4d.DTYPE_FILENAME:
            if bl2d[paramid] is not None and bl2d[paramid].strip() != "":
                if isVerbose:
                    print(indent + indentStep + bc[c4d.DESC_NAME] + ": " + bl2d[paramid]) # Print parameter name and filename

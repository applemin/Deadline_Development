...
class RB_KeyshotPlugin( DeadlinePlugin ):
    ...
    def RenderArgument( self ):
        ...
        writer.WriteLine()
        writer.WriteLine("import threading")
        writer.WriteLine("def RenderProcess():")
        writer.WriteLine("      threading.Timer(5.0, RenderProcess).start()")
        writer.WriteLine("      print(True)")

        writer.WriteLine("def JobQueue():")
        ...
        writer.WriteLine("threadA = threading.Thread(target=JobQueue)")
        writer.WriteLine("threadB = threading.Thread(target=RenderProcess)")
        writer.WriteLine("threadB.run()")
        writer.WriteLine("threadA.run()")
        #writer.WriteLine("JobQueue()")

...
def __main__():
    ...
    scriptDialog.SetTitle( "Setup Corona DR With Deadline" )
    scriptDialog.SetIcon( scriptDialog.GetIcon( 'CoronaDR' ) )
    ...
    scriptDialog.AddControlToGrid( "PoolBox", "PoolComboControl", "vray", 5, 1 )
    ...
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "Corona DR Options", 14, 0, colSpan=3 )

    scriptDialog.AddControlToGrid( "MaximumDrServersLabel", "LabelControl", "Maximum DrServers", 15, 0, "The maximum number of Corona DrServers to reserve for distributed rendering.", False )
    scriptDialog.AddRangeControlToGrid( "MaximumDrServersBox", "RangeControl", 10, 1, 100, 0, 1, 15, 1 )
    ...
    settings = ("DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","TaskTimeoutBox","MachineLimitBox","IsBlacklistBox","MachineListBox","LimitGroupBox","IsInterruptible","MaximumDrServersBox")
...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "CoronaDRSettings.ini" )

def SubmitButtonPressed(*args):
    global scriptDialog

    # Create job info file.
    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "corona_dr_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    writer.WriteLine( "Plugin=CoronaDR" )
    writer.WriteLine( "Frames=0-%s\n" % (scriptDialog.GetValue( "MaximumDrServersBox" )-1) )

    jobName = scriptDialog.GetValue( "NameBox" )
    jobName += " - Corona DR Job"
    ...
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "corona_dr_plugin_info.job" )

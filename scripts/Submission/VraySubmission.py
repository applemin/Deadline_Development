...
try:
    # python 3 doesn't have the built-in 'reload'
    from importlib import reload
except ImportError:
    # python 2, so use the built-in 'reload
    pass
...
def SubmitButtonPressed( *args ):
    ...
    if parsingSceneFile:
        ...
        if pulledOutputFiles is None:
            ...
            if framePlaceholderInDirectory:
                warnings.append('The output directory %s contains a frame number placeholder ("#"). Deadline does not support viewing job output in the monitor for such jobs.\n\n' % outputDir)

            if len(re.findall('#', outputFile)) > 1:
                warnings.append('The output path %s contains multiple hash ("#") characters. VRay will substitute each one with the frame number.\n\n' % outputFile)
            ...
            pulledOutputFiles = countedOutputFiles.keys() # No duplicates
...
class deadlineParsingProgressDialog(DeadlineScriptDialog):
    ...
    def threadTarget( self, filename, PathOverride ):
        try:
            ...
            self.ShowMessageBox( inst.message, "Parsing Error" )
        finally:

...
def __main__():
    ...
    scriptDialog.AddSelectionControlToGrid( "Input0Box", "FileBrowserControl", "", "AVI Files (*.avi);;M2V Files (*.m2v);;MPG Files (*.mpg);;VOB Files (*.vob);;WAV Files (*.wav);;All Files (*)", 1, 1 )
    ...
    scriptDialog.AddSelectionControlToGrid( "Input0ReplacePaddingBox", "CheckBoxControl", True, "Replace Frame in Input File(s) With Padding (file%03d.ext)", 3, 1, "If enabled, the frame number in the file name will be replaced by frame padding before being passed to FFMpeg. This should be enabled if you are passing a sequence of images as input." )

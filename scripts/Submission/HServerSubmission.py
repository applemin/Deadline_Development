...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "HServerSettings.ini" )

def SubmitButtonPressed(*args):
    global scriptDialog

    # Create job info file.
    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "hserver_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    writer.WriteLine( "Plugin=HServer" )

    jobName = scriptDialog.GetValue( "NameBox" )
    if jobName == "":
        jobName = "HServer Reserve Job"
    else:
        jobName += " - HServer Reserve Job"
    ...
    writer.WriteLine( "Frames=0-%s" % (scriptDialog.GetValue("ReserveCountBox")-1) )
    writer.WriteLine( "ChunkSize=1" )
    writer.Close()
    
    # Create plugin info file.
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "hserver_plugin_info.job" )

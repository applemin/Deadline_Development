...
def __main__():
    ...
    scriptDialog.SetTitle( "Setup Luxconsole Slave With Deadline" )
    scriptDialog.SetIcon( scriptDialog.GetIcon( 'LuxSlave' ) )
    ...
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "Luxconsole Slave Options", 14, 0, colSpan=3 )

    scriptDialog.AddControlToGrid( "MaximumSlavesLabel", "LabelControl", "Maximum LC Slaves", 15, 0, "The maximum number of Luxconsole Slaves to reserve for distributed rendering.", False )
    scriptDialog.AddRangeControlToGrid( "MaximumSlavesBox", "RangeControl", 10, 1, 100, 0, 1, 15, 1 )

    scriptDialog.AddControlToGrid( "PortNumberLabel", "LabelControl", "Port Number", 16, 0, "Override the default Luxconsole Slave TCP port number of 18018 to use.", False )
    scriptDialog.AddRangeControlToGrid( "PortNumberBox", "RangeControl", 18018, 1024, 65535, 0, 1, 16, 1 )

    scriptDialog.AddControlToGrid( "ThreadsLabel", "LabelControl", "Threads", 17, 0, "The number of threads to use. Specify 0 to use the same number of threads as there are CPUs.", False )
    scriptDialog.AddRangeControlToGrid( "ThreadsBox", "RangeControl", 0, 0, 256, 0, 1, 17, 1 )

    scriptDialog.AddControlToGrid( "VerbosityLabel", "LabelControl", "Verbosity Level", 18, 0, "The level of verbosity to use.", False )
    scriptDialog.AddComboControlToGrid( "VerbosityBox", "ComboControl", "None", ("None","Verbose (Debug)"), 18, 1 )
    ...
    settings = ("DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","TaskTimeoutBox","MachineLimitBox","IsBlacklistBox","MachineListBox","LimitGroupBox","IsInterruptible","MaximumSlavesBox","PortNumberBox","ThreadsBox","VerbosityBox")
...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "LuxSlaveSettings.ini" )
    
def SubmitButtonPressed(*args):
    global scriptDialog

    # Create job info file.
    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "luxslave_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    writer.WriteLine( "Plugin=LuxSlave" )
    writer.WriteLine( "Frames=0-%s\n" % (scriptDialog.GetValue( "MaximumSlavesBox" )-1) )
    
    jobName = scriptDialog.GetValue( "NameBox" )
    jobName += " - LuxConsole Slave Job"
    ...
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "luxslave_plugin_info.job" )

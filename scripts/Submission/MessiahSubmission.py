...
def __main__( *args ):
    ...
    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "Messiah Options", 0, 0, colSpan=3 )

    scriptDialog.AddControlToGrid( "SceneLabel", "LabelControl", "Messiah File", 1, 0, "The scene file to be rendered.", False )
    scriptDialog.AddSelectionControlToGrid( "SceneBox", "FileBrowserControl", "", "Messiah Files (*.fxs);;All Files (*)", 1, 1, colSpan=2 )

    scriptDialog.AddControlToGrid( "ContentLabel", "LabelControl", "Content Folder (Optional)", 2, 0, "The content folder that contains the assets used by the scene file. Using a content folder is recommended for network rendering, but can be left blank.", False )
    scriptDialog.AddSelectionControlToGrid( "ContentBox", "FolderBrowserControl", "", "", 2, 1, colSpan=2 )

    scriptDialog.AddControlToGrid( "OutputLabel", "LabelControl", "Output Folder (Optional)", 3, 0, "Override the output folder where the buffers will be saved. This is optional, and can be left blank. Note that when specified, all enabled buffers will be saved to this folder.", False )
    ...
    scriptDialog.AddSelectionControlToGrid("SubmitSceneBox","CheckBoxControl", False,"Submit Messiah Scene With Job", 5, 2, "If this option is enabled, the scene file will be submitted with the job, and then copied locally to the slave machine during rendering." )
    ...
    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator4", "SeparatorControl", "Frame Resolution", 0, 0, colSpan=3 )

    scriptDialog.AddControlToGrid( "WidthLabel", "LabelControl", "Width (0 to use default)", 1, 0, "The width of the image in pixels (specify 0 to use the default width).", False )
    scriptDialog.AddRangeControlToGrid( "WidthBox", "RangeControl", 0, 0, 1000000, 0, 1, 1, 1 )
    scriptDialog.AddHorizontalSpacerToGrid("AOHSpacer1", 1, 2)

    scriptDialog.AddControlToGrid( "HeightLabel", "LabelControl", "Height (0 to use default)", 2, 0, "The height of the image in pixels (specify 0 to use the default height).", False )
    scriptDialog.AddRangeControlToGrid( "HeightBox", "RangeControl", 0, 0, 1000000, 0, 1, 2, 1 )

    scriptDialog.AddControlToGrid( "Separator5", "SeparatorControl", "Antialiasing", 3, 0, colSpan=3 )

    aaOverrideBox = scriptDialog.AddSelectionControlToGrid("OverrideAABox","CheckBoxControl",False,"Override Antialiasing Settings", 4, 0, "Enable to override the antialiasing mode and level.", colSpan=2)
    aaOverrideBox.ValueModified.connect(AAOverideChanged)

    scriptDialog.AddControlToGrid( "AAModeLabel", "LabelControl", "Antialiasing Mode", 5, 0, "The antialiasing mode to use.", False )
    scriptDialog.AddComboControlToGrid( "AAModeBox", "ComboControl", "Adaptive", ("Off","Adaptive","Enhanced"), 5, 1 )

    scriptDialog.AddControlToGrid( "AALevelLabel", "LabelControl", "Antialiasing Level", 6, 0, "The antialiasing level.", False )
    scriptDialog.AddRangeControlToGrid( "AALevelBox", "RangeControl", 3, 1, 10, 0, 1, 6, 1 )
    scriptDialog.EndGrid()
    scriptDialog.EndTabPage()
    
    integration_dialog = IntegrationUI.IntegrationDialog()
    integration_dialog.AddIntegrationTabs( scriptDialog, "MessiahMonitor", DraftRequested, ProjectManagementOptions, failOnNoTabs=False )
    ...
    settings = ("DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","MachineLimitBox","IsBlacklistBox","MachineListBox","LimitGroupBox","SceneBox","FramesBox","ChunkSizeBox","ContentBox","OutputBox","ThreadsBox","BuildBox")
    ...
    if len( args ) > 0:
        ...
        if args[0] == "":
            scriptDialog.ShowMessageBox( "The Messiah scene must be saved before it can be submitted to Deadline.", "Error" )
            return
...
def AAOverideChanged(*args):
    global scriptDialog
    enabled = scriptDialog.GetValue( "OverrideAABox" )
    scriptDialog.SetEnabled( "AAModeLabel", enabled )
    scriptDialog.SetEnabled( "AAModeBox", enabled )
    scriptDialog.SetEnabled( "AALevelLabel", enabled )
    scriptDialog.SetEnabled( "AALevelBox", enabled )

def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "MessiahSettings.ini" )
   
def SubmitButtonPressed(*args):
    global scriptDialog
    global integration_dialog
    
    # Check if Messiah files exist.
    sceneFile = scriptDialog.GetValue( "SceneBox" )
    if( not File.Exists( sceneFile ) ):
        scriptDialog.ShowMessageBox( "The Messiah file %s does not exist" % sceneFile, "Error" )
        return
    elif (not scriptDialog.GetValue("SubmitSceneBox") and PathUtils.IsPathLocal(sceneFile)):
        result = scriptDialog.ShowMessageBox( "The Messiah file %s is local and is not being submitted with the job. Are you sure you want to continue?" % sceneFile, "Warning", ("Yes","No") )
        if(result=="No"):
            return
    ...
    if contentFolder != "":
        if(not Directory.Exists(contentFolder)):
            scriptDialog.ShowMessageBox( "The content folder %s does not exist." % contentFolder, "Error" )
            return
        elif( PathUtils.IsPathLocal(contentFolder) ):
            result = scriptDialog.ShowMessageBox( "The content folder %s is local. Are you sure you want to continue?" % contentFolder, "Warning", ("Yes","No") )
            if(result=="No"):
                return
    ...
    if outputFolder != "":
        if(not Directory.Exists(outputFolder)):
            scriptDialog.ShowMessageBox( "The output folder %s does not exist." % outputFolder, "Error" )
            return
    ...
    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "messiah_job_info.job" )
    writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
    writer.WriteLine( "Plugin=Messiah" )
    ...
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "messiah_plugin_info.job" )
    ...
    if contentFolder != "":
        writer.WriteLine( "ContentFolder=%s" % contentFolder )
    if outputFolder != "":
    ...
    writer.WriteLine( "Width=%s" % scriptDialog.GetValue( "WidthBox" ) )
    writer.WriteLine( "Height=%s" % scriptDialog.GetValue( "HeightBox" ) )
    
    writer.WriteLine( "OverrideAntialiasing=%s" % scriptDialog.GetValue( "OverrideAABox" ) )
    writer.WriteLine( "AntialiasingMode=%s" % scriptDialog.GetValue( "AAModeBox" ) )
    writer.WriteLine( "AntialiasingLevel=%s" % scriptDialog.GetValue( "AALevelBox" ) )

...
def __main__( *args ):
    ...
    scriptDialog.SetTitle( "Submit Salt Update To Deadline" )
    scriptDialog.SetIcon( scriptDialog.GetIcon( 'Salt' ) )
    ...
    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "Salt Options", 0, 0, colSpan=3)

    scriptDialog.AddControlToGrid("LoggingLabel", "LabelControl", "Verbose Logging Level", 1, 0, "The level of verbose logging Salt will provide.", False)
    scriptDialog.AddComboControlToGrid( "LoggingComboBox", "ComboControl", "info", ["all", "garbage", "trace", "debug", "info", "warning", "error", "critical", "quiet" ], 1, 1)
    scriptDialog.EndGrid()
    ...
    settings = ( "DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","IsBlacklistBox","MachineListBox","LimitGroupBox", "LoggingComboBox" )
...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "SaltSettings.ini" )

def SubmitButtonPressed( *args ):
    global scriptDialog

    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "salt_job_info.job" )
    ...
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "salt_plugin_info.job" )
    writer = StreamWriter( pluginInfoFilename, False, Encoding.Unicode )
    writer.WriteLine("Logging=%s" % scriptDialog.GetValue("LoggingComboBox") )
    writer.Close()

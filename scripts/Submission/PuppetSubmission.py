...
def __main__( *args ):
    ...
    scriptDialog.SetTitle( "Submit Puppet Update To Deadline" )
    scriptDialog.SetIcon( scriptDialog.GetIcon( 'Puppet' ) )
    ...
    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "Puppet Options", 0, 0, colSpan=4)

    scriptDialog.AddSelectionControlToGrid( "VerboseCheckBox", "CheckBoxControl", False, "Verbose Logging", 1, 0, "If enabled, the puppet update will have verbose logging enabled.")
    scriptDialog.EndGrid()
    ...
    settings = ( "DepartmentBox","CategoryBox","PoolBox","SecondaryPoolBox","GroupBox","PriorityBox","IsBlacklistBox","MachineListBox","LimitGroupBox", "VerboseCheckBox" )
...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "PuppetSettings.ini" )

def SubmitButtonPressed( *args ):
    global scriptDialog

    jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "puppet_job_info.job" )
    ...
    writer.WriteLine( "Plugin=Puppet" )
    ...
    pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "puppet_plugin_info.job" )
    writer = StreamWriter( pluginInfoFilename, False, Encoding.Unicode )
    writer.WriteLine("Verbose=%s" % scriptDialog.GetValue("VerboseCheckBox") )
    writer.Close()

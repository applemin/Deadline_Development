...
def __main__( *args ):
    ...
    scriptDialog.AddControlToGrid( "NameBox", "TextControl", "No Job Name", 1, 1 )
    ...
    scriptDialog.AddGrid()
    scriptDialog.AddControlToGrid( "Separator3", "SeparatorControl", "CSi Bridge File Options", 0, 0, colSpan=3 )

    scriptDialog.AddControlToGrid( "SceneLabel", "LabelControl", "CSi Bridge Data File(s)", 1, 0, "The CSi Bridge Data File to be processed.", False )
    scriptDialog.AddSelectionControlToGrid( "SceneBox", "MultiFileBrowserControl", "", "CSi Bridge Files (*.BDB);;Microsoft Access Files (*.MDB);;Microsoft Excel Files (*.XLS);;CSi Bridge Text Files (*.$BR *.B2K)", 1, 1, colSpan=2 )
    ...
    scriptDialog.AddSelectionControlToGrid("SubmitSceneBox","CheckBoxControl",False,"Submit Data File With Job", 4, 2, "If this option is enabled, the Bridge file will be submitted with the job, and then copied locally to the slave machine during processing.")
    scriptDialog.AddControlToGrid( "VersionLabel", "LabelControl", "Version", 5, 0, "The version of CSiBridge to render with.", False )
    scriptDialog.AddComboControlToGrid( "VersionBox", "ComboControl", "2015", ("15", "2014", "2015"), 5, 1 )
    ...
    scriptDialog.AddControlToGrid( "ProcessLabel", "LabelControl", "Process Selection", 1, 0, "Choose to execute inside of the existing Bridge application process or as a separate process.", False )
    ...
    scriptDialog.AddControlToGrid( "CommandLineLabel", "LabelControl", "Command Line Args", 3, 0, "Additional command line flags/options can be added here if required." )
...
def GetSettingsFilename():
    return Path.Combine( ClientUtils.GetUsersSettingsDirectory(), "CSiBridgeSettings.ini" )   
...
def SubmitButtonPressed(*args):
    ...
    try:
        # Check if Bridge file(s) exist.
        dataFiles = StringUtils.FromSemicolonSeparatedString( scriptDialog.GetValue( "SceneBox" ), False )

        if( len( dataFiles ) == 0 ):
            scriptDialog.ShowMessageBox( "No CSi Bridge data file specified", "Error" )
            return

        for dataFile in dataFiles:
            if( not File.Exists( dataFile ) ):
                scriptDialog.ShowMessageBox( "The CSi Bridge data file '%s' does not exist" % dataFile, "Error" )
                return
            elif (not scriptDialog.GetValue("SubmitSceneBox") and PathUtils.IsPathLocal(dataFile)):
                scriptDialog.ShowMessageBox( "The CSi Bridge data file is local and 'Submit Data File with Job' is disabled.\n\nAll local data files must be submitted with the job.", "Error" )
                return
        ...
        ## Check that if Override Output Directory is enabled, that Bridge Data File Directory != Override Output Directory!
        if ( bool( scriptDialog.GetValue( "OverrideOutput" ) ) == True ):
            for dataFile in dataFiles:
        ...
        for dataFile in dataFiles:         
            ...
            jobInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "csibridge_job_info.job" )
            writer = StreamWriter( jobInfoFilename, False, Encoding.Unicode )
            writer.WriteLine( "Plugin=CSiBridge" )
            ...
            pluginInfoFilename = Path.Combine( ClientUtils.GetDeadlineTempPath(), "csibridge_plugin_info.job" )

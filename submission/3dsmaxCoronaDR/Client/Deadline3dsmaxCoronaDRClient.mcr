macroScript MaxCoronaDR2Deadline category:"Deadline" buttontext:"Deadline (Corona DR)" tooltip:"Setup Corona DR With Deadline"
(

	local messageTitle = "Deadline (Corona DR)"
	local goOn = true
	...
	(
		...
		(
			...
			local commandArguments = "-outputfiles \"" + submitOutputFile + "\" \"" + submitExitCodeFile + "\" -getrepositorypath submission/3dsmaxCoronaDR/Main"
		...
		(
			 try 
			 ( 
				mainScript = theNetworkRoot + "/3dsmaxCoronaDR2Deadline.ms"
				format "Running submission script \"%\"\n" mainScript
							
				if doesFileExist mainScript then
				(
					format "3dsmaxCoronaDR2Deadline Script File FOUND in the Deadline Repository...\n"
					fileIn mainScript quiet:true
					format "3dsmaxCoronaDR2Deadline Macro UPDATED.\n"
				)
				else
					messageBox "Failed To Update 3dsmaxCoronaDR2Deadline Script from Deadline Repository. (error code: 1005)"
			) 
			catch
		...
		macros.run "Thinkbox" "MaxCoronaDR2DeadlineMain"
	)
)
